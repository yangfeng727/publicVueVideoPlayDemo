// import H265webjsModule from '../../../../static/h265webjsOther/index'
...
export function makeH265webjs(videoURL, config, wrapBox) {
  ...
  let playAction = () => {
    ...
    if (playerObj.isPlaying()) {
      ...
      playerObj.pause();
      configEventCallback.pause && configEventCallback.pause() // 回调
    } else {
      // playBar.textContent = '||';
      playBar.setAttribute('class', 'pauseBtn');
      playerObj.play();
      configEventCallback.play && configEventCallback.play() // 回调
    }
  };
  ...
  fullScreenBtn.onclick = () => {
    if (playerDom.firstChild) {
      playerDom.firstChild.requestFullscreen();
    } else {
  ...
  playerObj.onPlayTime = (videoPTS) => {
    // 回调，参数（当前播放时间,总缓冲时间）
    configEventCallback.onPlayTime && configEventCallback.onPlayTime(videoPTS, catchTime)
    // console.log(videoPTS, catchTime, playerObj.mediaInfo().meta.durationMs, 111)
